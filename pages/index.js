...
const Home = () => {
  ...
  const loadingClass = loaded ? 'hidden' : 'visible'
  const loadingPlaceholder = (
    <div className={`loading text-4xl my-8 ${loadingClass}`}>
      Loading...
    </div>
  )

  const postClass = loaded && posts.length ? 'visible' : 'hidden'
  ...
  const notFoundClass = loaded && !posts.length ? 'visible' : 'hidden'
  ...
  const pageContainer = !loaded ? loadingPlaceholder : posts.length ? postsContainer : notFoundContainer
